...
export default function AIAssistantTab({ roomId }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {/* Header */}
      <div style={{ padding: "16px 24px", borderBottom: `1px solid ${theme.colors.border}`, background: "linear-gradient(135deg,#F3F0FF 0%,#FCE7F3 100%)", display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: theme.colors.primaryGradient, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <RiRobot2Fill size={22} color="#fff" />
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: theme.colors.textPrimary }}>Study Copilot AI</p>
          <p style={{ margin: 0, fontSize: 12, color: theme.colors.textMuted }}>Powered by Gemini · Based on your study materials</p>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${theme.colors.border}`, background: "#fff", padding: "0 24px" }}>
        {[
          { key: "chat",      label: "Ask AI",    Icon: IoChatbubblesOutline },
          { key: "summarize", label: "Summarize", Icon: HiOutlineDocumentText },
          { key: "quiz",      label: "Quiz",      Icon: MdOutlineQuiz },
        ].map(({ key, label, Icon }) => (
          <button
            key={key}
            onClick={() => { setActiveMode(key); setResult(null); }}
            style={{
              display: "flex", alignItems: "center", gap: 6,
              padding: "12px 20px", border: "none", background: "transparent", cursor: "pointer",
              fontSize: 13, fontWeight: activeMode === key ? 700 : 400,
              color: activeMode === key ? theme.colors.primary : theme.colors.textMuted,
              borderBottom: activeMode === key ? `2.5px solid ${theme.colors.primary}` : "2.5px solid transparent",
              marginBottom: -1, transition: "all 0.15s",
            }}
          >
            <Icon size={15} />
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
            {messages.map((msg, i) => (
              <div key={i} style={{ display: "flex", flexDirection: msg.role === "user" ? "row-reverse" : "row", gap: 10, alignItems: "flex-start" }}>
                {msg.role === "ai" && (
                  <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: theme.colors.primaryGradient, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            {loading && (
              <div style={{ display: "flex", gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 10, background: theme.colors.primaryGradient, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <RiRobot2Line size={18} color="#fff" />
                </div>
                <div style={{ padding: "12px 16px", borderRadius: "16px 16px 16px 4px", background: "#F3F0FF", color: theme.colors.textMuted, fontSize: 14 }}>Thinking…</div>
              </div>
          ...
          <div style={{ padding: "12px 20px", borderTop: `1px solid ${theme.colors.border}`, display: "flex", gap: 10, background: "#fff" }}>
            ...
            <button
              onClick={askQuestion}
              disabled={loading || !query.trim()}
              style={{ width: 42, height: 42, borderRadius: "50%", border: "none", background: loading ? "#C4B5FD" : theme.colors.primaryGradient, color: "#fff", cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
              <IoSendSharp size={17} />
            </button>
          </div>
          ...
          {resources.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: theme.colors.textMuted }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
                <AiOutlineFilePdf size={48} color={theme.colors.textMuted} />
              </div>
              <p style={{ fontSize: 15, fontWeight: 600, color: theme.colors.textSecondary }}>No PDFs uploaded yet</p>
              <p style={{ fontSize: 13 }}>Upload a PDF in the Resources tab first</p>
            </div>
            ...
            <>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {resources.map((r) => (
                    ...
                    >
                      <AiOutlineFilePdf size={24} color={selectedFile?.id === r.id ? theme.colors.primary : theme.colors.textMuted} />
                      <div>
                        <p style={{ margin: 0, fontWeight: 600, fontSize: 13, color: theme.colors.textPrimary }}>{r.title}</p>
                        <p style={{ margin: 0, fontSize: 11, color: theme.colors.textMuted }}>{r.originalFileName}</p>
                      </div>
              ...
              {result && (
                <div style={{
                  ...
                  {result.type !== "error" && (
                    <p style={{ margin: "0 0 12px", fontWeight: 700, fontSize: 13, color: theme.colors.textMuted, display: "flex", alignItems: "center", gap: 6 }}>
                      {result.type === "summary"
